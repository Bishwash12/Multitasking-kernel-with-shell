...
struct command_argument* pearos_parse_command(const char* command, int max)
{
    ...
    root_command = pearos_malloc(sizeof(struct command_argument));
    if (!root_command)
    {
        goto out;
    }
    ...
    while (token != 0)
    {
        struct command_argument* new_command = pearos_malloc(sizeof(struct command_argument));
        if(!new_command)
        {
            break;
        }
...
int pearos_system_run(const char* command)
{
    ...
    struct command_argument* root_command_argument = pearos_parse_command(command, sizeof(buf));
    ...
    return pearos_system(root_command_argument);
}